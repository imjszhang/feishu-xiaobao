...
class FeishuDocxContentManager:
    ...
    def _get_parent_info(self, document_id: str, target_block_id: str) -> tuple:
        """
        获取父块ID和目标块索引
        ...
        """
        # 获取目标块的信息
        target_block_info = self.docx_handler.get_block_contents(document_id, target_block_id)
        ...
        # 获取父块的所有子块
        parent_children = self.docx_handler.get_block_children(document_id, parent_id)
    ...
    def _add_single_callout(self, 
        ...
        try:
            ...
            callout_response = self.docx_handler.create_block(
            ...
            # 添加子块到callout中
            child_response = self.docx_handler.create_block(
                document_id=document_id,
            ...
            callout_children = self.docx_handler.get_block_children(document_id, callout_block_id)
            time.sleep(1)  # 添加等待
            
            if callout_children and callout_children.get('code') == 0:
                delete_response = self.docx_handler.delete_block(
                    document_id=document_id,
                    block_id=callout_block_id,
                    start_index=0,
                    end_index=1
    ...
    def add_content_blocks(self, 
        ...
        try:
            # 获取父块信息
            parent_id, target_index = self._get_parent_info(document_id, target_block_id)
            time.sleep(1)  # 添加等待
            ...
            for content in contents:
                next_index = self._add_single_callout(
                    document_id=document_id,
                    parent_id=parent_id,
            ...
            heading_response = self.docx_handler.create_block(
                document_id=document_id,
                block_id=parent_id,
...
def update_feishu_xiaobao_post(payload: dict = Body(...)):
    ...
    # 添加内容块
    result = content_manager.add_content_blocks(DOC_ID, TARGET_BLOCK_ID, DATE_STR, CONTENT_DATA)
    if result:
